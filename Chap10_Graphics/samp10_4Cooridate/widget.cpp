...
{
    QString str=QString::asprintf("Graphics view坐标，左上角为（0,0），宽度= %d， 高度=%d",ui->graphicsView->width(),ui->graphicsView->height());
    ui->labViewSize->setText(str);

    QRectF rectF=ui->graphicsView->sceneRect();     // 场景框
    str=QString::asprintf("QGraphicsView::sceneRect=(left, top, width, Height) = %.0f, %.0f, %.0f, %.0f", rectF.left(),rectF.top(), rectF.width(), rectF.height());
